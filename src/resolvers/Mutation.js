...
async function createProjectActivity(_parent, args, { prisma, req }, _info) {
  if (getUser(req).userId) {
    const { input } = args

    const ProjectActivityCreateInput = {
      ...input,
      project: { connect: { id: parseInt(input.projectId) } },
      socialWorker: { connect: { id: parseInt(input.socialWorkerId) } }
    }
    delete ProjectActivityCreateInput.projectId
    delete ProjectActivityCreateInput.socialWorkerId
...
async function updateProjectActivity(_parent, args, { prisma, req }, _info) {
  if (getUser(req).userId) {
    const { input } = args
    const { id } = input
    delete input.id

    const previousProject = await prisma.projectActivity
      .findUnique({ where: { id: parseInt(id) } })
      .project()
    const previousProjectId = previousProject ? previousProject.id : undefined

    const ProjectActivityUpdateInput = {
      ...input,
      project: previousProjectId
        ? { connect: { id: previousProjectId } }
        : undefined
    }
    return await prisma.projectActivity.update({
...
async function createDailyActivity(_parent, args, { prisma, req }, _info) {
  if (getUser(req).userId) {
    const { input } = args

    const DailyActivityCreateInput = {
      ...input,
      projectActivity: { connect: { id: parseInt(input.projectActivityId) } }
    }
    delete DailyActivityCreateInput.projectActivityId
...
async function updateDailyActivity(_parent, args, { prisma, req }, _info) {
  if (getUser(req).userId) {
    const { input } = args
    const { id } = input
    delete input.id

    const previousProjectActivity = await prisma.dailyActivity
      .findUnique({ where: { id: parseInt(id) } })
      .projectActivity()
    const previousProjectActivityId = previousProjectActivity
      ? previousProjectActivity.id
      : undefined
    const previousSocialWorker = await prisma.dailyActivity
      .findUnique({ where: { id: parseInt(id) } })
      .projectActivity()
    const previousSocialWorkerId = previousSocialWorker
      ? previousSocialWorker.id
    ...
    const DailyActivityUpdateInput = {
      ...input,
      projectActivity: previousProjectActivityId
        ? { connect: { id: previousProjectActivityId } }
        : undefined,
      socialWorker: previousSocialWorkerId
        ? { connect: { id: previousSocialWorkerId } }
        : undefined
    }
